...
func CrearArregloDeModulos2048(arregloBool []bool, cantModulos int) [][2048]bool {
	arregloModulos := make([][constants.TAM_BITS_TOTALES_MODULO_2048]bool, cantModulos)

	contadorModulo := 0

	for i := 0; i < len(arregloBool); i += constants.TAM_BITS_TOTALES_MODULO_2048 {
		indice := 0

		for j := i; j < i+constants.TAM_BITS_TOTALES_MODULO_2048; j++ {
...
func ChequearErrorModulo2048(modulo [2048]bool, matriz1024 [2048][11]bool) int {
	result := make([]bool, constants.TAM_BITS_CONTROL_MODULO_2048)

	for columna := 0; columna < constants.TAM_BITS_CONTROL_MODULO_2048; columna++ {

		cantidadDeUnos := 0

		for fila := 0; fila < constants.TAM_BITS_TOTALES_MODULO_2048; fila++ {
